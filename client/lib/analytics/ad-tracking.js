...
import { assign, clone, cloneDeep, includes } from 'lodash';
...
export function recordSignup( slug ) {
	...
	const wpcomJetpackCartInfo = splitWpcomJetpackCartInfo( syntheticCart );

	// AdWords:

	if ( isAdwordsEnabled ) {
		if ( window.google_trackConversion ) {
			let googleConversionId, googleConversionLabel, cost;
	...
	if ( isWpcomGoogleAdsGtagEnabled ) {
		if ( wpcomJetpackCartInfo.containsWpcomProducts ) {
			...
			window.gtag( 'event', 'conversion', event_data );
			debug( 'recordSignup: [Google Ads Gtag] record WPCom signup', event_data );
		}
	}

	// Google Ads Gtag: Jetpack

	// We do not track signups on Jetpack account only on the WordPress.com
	if ( isJetpackGoogleAdsGtagEnabled ) {
		if ( wpcomJetpackCartInfo.containsJetpackProducts ) {
			const event_data = {
				send_to: TRACKING_IDS.jetpackGoogleAdsGtagSignup,
				value: wpcomJetpackCartInfo.jetpackCost,
				currency: syntheticCart.currency,
				transaction_id: syntheticOrderId,
			};
			window.gtag( 'event', 'conversion', event_data );
			debug( 'recordSignup: [Google Ads Gtag] record Jetpack signup', event_data );
		}
	}
	...
	if ( isBingEnabled ) {
		...
		if ( wpcomJetpackCartInfo.containsJetpackProducts ) {
			if ( null !== wpcomJetpackCartInfo.jetpackCostUSD ) {
				...
				window.uetq.push( bingParams );
				debug( 'recordSignup: [Bing] record Jetpack signup', bingParams );
			} else {
				debug( 'recordSignup: [Bing] currency not supported, dropping Jetpack pixel' );
			}
		}
	}
	...
	if ( isFacebookEnabled ) {
		...
		if ( wpcomJetpackCartInfo.containsJetpackProducts ) {
			...
			window.fbq( 'trackSingle', TRACKING_IDS.facebookJetpackInit, 'Subscribe', fbParams );
			debug( 'recordSignup: [Facebook] record Jetpack signup', fbParams );
		}
	}
	...
	if ( isFloodlightEnabled ) {
		// NOTE: we do not distinguish between WPCom and Jetpack here
		const params = {
			type: 'wordp0',
			cat: 'signu1',
		};
...
function splitWpcomJetpackCartInfo( cart ) {
	...
	return {
		...
		containsJetpackProducts: 0 !== wpcomProducts.length(),
		containsWpcomProducts: 0 !== jetpackProducts.length(),
...
function recordOrderInDonutsGtag( cart, orderId ) {
	const allowed = isAdTrackingAllowed() && isDonutsGtagEnabled;
	debug( 'recordOrderInDonutsGtag' + ( allowed ? '' : ' [Disallowed]' ), cart, orderId );
	if ( ! allowed ) {
		return;
	}
...
export function recordSignupStartInDonutsGtag() {
...
function recordOrderInGoogleAds( cart, orderId, wpcomJetpackCartInfo ) {
	...
	if ( isJetpackGoogleAdsGtagEnabled ) {
		if ( wpcomJetpackCartInfo.containsJetpackProducts ) {
			...
			window.gtag( 'event', 'conversion', event_data );
			debug( 'recordOrderInGoogleAds: Record Jetpack Purchase', event_data );
		}
	}
	...
	if ( isWpcomGoogleAdsGtagEnabled ) {
		if ( wpcomJetpackCartInfo.containsWpcomProducts ) {
			...
			window.gtag( 'event', 'conversion', event_data );
			debug( 'recordOrderInGoogleAds: Record WPCom Purchase', event_data );
		}
	}
}
